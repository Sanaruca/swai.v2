...
export const generar_constancia_de_estudios = async (estudiante: EstudianteDTO) => {
  console.log("Generando constancia para:", `${estudiante.nombres} ${estudiante.apellidos}`);
  ...
  try {
    ...
    doc.text("MINISTERIO DEL PODER POPULAR PARA LA EDUCACIÓN", pageWidth / 2, 28, { align: "center" });
    doc.text("COMPLEJO EDUCATIVO MIGUEL JOSÉ SANZ", pageWidth / 2, 36, { align: "center" });
    doc.text("MATURÍN MONAGAS", pageWidth / 2, 44, { align: "center" });
    
    // Salto de línea y título
    doc.text("CONSTANCIA DE ESTUDIOS", pageWidth / 2, 60, { align: "center" });
    ...
    const contentText = 
      ...
      `${estudiante.nivel_academico.nombre}.`;
    ...
    // Firma centrada
    doc.text("Prof. Luis E. Martinez P.", pageWidth / 2, signatureY, { align: "center" });
    doc.text("CI: V-15.788.669", pageWidth / 2, signatureY + 8, { align: "center" });
    doc.text("DIRECTOR(E)", pageWidth / 2, signatureY + 16, { align: "center" });
    doc.text("Telf. 0412-0879640", pageWidth / 2, signatureY + 24, { align: "center" });
    
    // Guardar el PDF y descargarlo
    doc.save(`constancia_estudios_${estudiante.cedula}.pdf`);
