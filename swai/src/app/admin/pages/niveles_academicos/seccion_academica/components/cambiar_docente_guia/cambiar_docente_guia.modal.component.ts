...
@Component({
  ...
  styleUrl: './cambiar_docente_guia.modal.component.css',
})
export class CambiarDocenteGuiaModalComponent implements OnDestroy {
  ...
  open() {
    this.visible = true;
    this.load_data().then(() => {
      this.subs.busqueda = this.busqueda.valueChanges.subscribe((busqueda) => {
  ...
  protected async save() {
    ...
    try {
      const profesor =
        (await this.api.client.empleados.actualizar_empelado.mutate({
