...
import {
    CantidadDeEstudiantesPorNivelAcademicoDTO,
    Paginated,
} from '@swai/server';
...
@Component({
  ...
  styleUrl: './nivel_academico.page.component.css',
})
export class NivelAcademicoPageComponent {
  ...
  protected acciones_menu: MenuItem[] = [
    ...
    },
    {
      ...
      icon: 'pi pi-download',
      command: () => {
        this.exportar_estudiantes_modal().open();
      },
    },
  ];
  ...
  protected pensum_modal = viewChild.required(PensumModalComponent);
  ...
  protected secciones_academicas = this.route.snapshot.data[
    'secciones_academicas'
  ] as Paginated<SeccionDTO>;
