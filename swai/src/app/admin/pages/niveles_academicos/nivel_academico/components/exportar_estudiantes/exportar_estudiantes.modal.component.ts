...
import {
    ESTADO_ACADEMICO,
    generar_matricula_de_estudiantes,
    Matricula,
...
@Component({
  ...
  styleUrl: './exportar_estudiantes.modal.component.css',
})
export class ExportarEstudiantesModalComponent {
  ...
  protected async exportar() {
    ...
    try {
      const secciones_academicas =
        await this.api.client.niveles_academicos.obtener_secciones_academicas.query(
          { nivel_academico: this.nivel_academico() }
        );

      secciones_academicas.data.forEach((seccion) => {
        matriculas.push({
      ...
      await generar_matricula_de_estudiantes(
        matriculas,
        `matriculas_${NIVEL_ACADEMICO_CARDINAL_MAP[this.nivel_academico()]}`
      );
      this.success.emit();
