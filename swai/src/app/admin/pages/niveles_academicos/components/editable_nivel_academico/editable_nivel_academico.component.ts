...
@Component({
  ...
  styleUrl: './editable_nivel_academico.component.css',
})
export class EditableNivelAcademicoComponent implements OnInit {
  ...
  protected secciones_editables: Array<
    Seccion & { aux: { existe: boolean; add: boolean } }
  > = [];

  get puede_aplicar_cambios() {
    return (
      this.secciones_adicionales.length > 0 ||
      this.secciones_editables.some((seccion) => seccion.aux.add)
    );
  }
  get_secciones_editables(): Array<
    Seccion & { aux: { existe: boolean; add: boolean } }
  > {
    const secciones = this.short_secciones(
    ...
    const secciones_editables: Array<
      Seccion & { aux: { existe: boolean; add: boolean } }
    > = [];
    ...
    for (let i = start; i <= end; i++) {
      ...
      const seccion = existe
        ? secciones.find((s) => s.seccion === seccionValue)!
        : ({
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.api.client.secciones_academicas.eliminar_seccion_academica
          .mutate(seccion.id)
          .then(() => {
            const secciones_para_añadir = this.secciones_editables.filter(
              (seccion) => !seccion.aux.existe && seccion.aux.add
            );

            this.secciones_editables = this.secciones_editables.filter(
              (it) => it.id !== seccion.id
            );

            this.load_data().then(() => {
              secciones_para_añadir.forEach((seccion) => {
                const index = this.secciones_editables.findIndex(
                  (it) => it.id === seccion.id
                );
                if (index > -1) {
  ...
  cancelar() {
    this.secciones_adicionales = [];
    this.secciones_editables = this.secciones_editables.map((seccion) => ({
      ...seccion,
      aux: { ...seccion.aux, add: false },
    }));
    this.editing = false;
  }
  ...
  protected aplicar_cambios() {
    ...
    const secciones_adicionales = [
      ...this.secciones_adicionales,
      ...this.secciones_editables.filter(
        (seccion) => !seccion.aux.existe && seccion.aux.add
      ),
    ] as Seccion[];
  ...
  protected short_secciones(secciones: NivelAcademico['secciones']) {
    const copy = JSON.parse(
      JSON.stringify(secciones)
    ) as NivelAcademico['secciones'];
  ...
  private async load_data() {
    ...
    try {
      ...
      const cantidad_de_estudiantes_del_nivel_academico =
        cantidad_de_estudiantes.niveles_academicos.find(
      ...
      this.nivel_academico_data = {
        ...
        cantidad_de_estudiantes: {
          ...
          no_inscritos:
            cantidad_de_estudiantes_del_nivel_academico?.no_inscritos ?? 0,
          retirados:
            cantidad_de_estudiantes_del_nivel_academico?.retirados ?? 0,
