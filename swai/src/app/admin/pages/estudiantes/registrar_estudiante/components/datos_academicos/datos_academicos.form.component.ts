...
@Component({
  ...
  styleUrl: './datos_academicos.form.component.css',
})
export class DatosAcademicosFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.datos_academicos.controls.posee_materias_pendientes.valueChanges.subscribe(
      (posee_materias_pendientes) => {
        if (posee_materias_pendientes) {
          this.datos_academicos.controls.materias_pendientes.setValidators([
            Validators.required,
          ]);
        } else {
