...
@Component({
  ...
  styleUrl: './registrar_estudiante.page.component.css',
})
export class RegistrarEstudiantePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.data.subscribe((data) => {
      ...
      const nivel_academico = Number(
        this.route.snapshot.queryParamMap.get('nivel_academico')
      );
      ...
      if (modo === 'registrar' && nivel_academico) {
        ...
        this.datos_academicos_component().form.controls.nivel_academico.setValue(
          nivel_academico
        );
      }
    });
