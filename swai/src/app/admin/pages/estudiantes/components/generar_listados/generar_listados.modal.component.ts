...
@Component({
  ...
  styleUrl: './generar_listados.modal.component.css',
})
export class GenerarListadosModalComponent implements OnInit {
      ...
      Promise.all(
        ...
        .then((estudiantes) => {
          ...
          estudiantes.forEach((estudiantes) => {
            ...
            const filename = `listado_de_estudiantes_${nivel_academico.nombre
              .toLowerCase()
              .replace(/\/s/g, '_')}`;
  ...
  protected get secciones_selecionadas(): string {
    ...
    return Object.keys(this.form.controls.secciones_academicas.value)
      .filter(
        (key) => this.form.controls.secciones_academicas.value[key]?.selected
      )
      .join(', ');
  }
  ...
  protected get niveles_selecionados(): string {
    ...
    return this.form.controls.niveles_academicos.controls
      .filter((control) => control.get('selected')?.value)
      .map((control) => control.get('nivel_academico')?.value)
      .join(', ');
  }
