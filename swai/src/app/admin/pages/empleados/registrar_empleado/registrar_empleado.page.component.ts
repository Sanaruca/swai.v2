...
@Component({
  ...
  styleUrl: './registrar_empleado.page.component.css',
})
export class RegistrarEmpleadoPageComponent implements OnInit {
  ...
  protected niveles_academicos = this.route.snapshot.data[
    'niveles_academicos'
  ] as NivelAcademicoConSeccionesDTO[];
  ...
  get SECCIONES() {
    const nivel_academico =
      this.datos_de_empleo.controls.docente_guia_nivel_academico.value;
  ...
  ngOnInit(): void {
    this.datos_de_empleo.controls.docente_guia.valueChanges.subscribe(
      (es_docente_guia) => {
    ...
    this.datos_de_empleo.controls.docente_guia_nivel_academico.valueChanges.subscribe(
      (nivel_academico) => {
        if (!nivel_academico) return;

        const seccion =
          this.datos_de_empleo.controls.docente_guia_seccion.value;
        if (!seccion) return;
        if (
          !this.SECCIONES.some(
            (it) =>
              it.nivel_academico === nivel_academico && it.seccion === seccion
          )
        ) {
  ...
  private init_modo(modo: 'registrar' | 'editar') {
    if (modo === 'editar') {
      ...
      this.datos_de_empleo.setValue({
        ...
        docente_guia_nivel_academico:
          (empleado as ProfesorDTO).seccion_guia?.nivel_academico || null,
        docente_guia_seccion:
          (empleado as ProfesorDTO).seccion_guia?.seccion || null,
      });
  ...
  private async registrar() {
    ...
    const seccion_guia = (
      es_docente_guia ? `${nivel_academico}${seccion}` : null
    ...
    try {
      this.api.client.empleados.actualizar_empelado.mutate({
        ...
        datos: {
          ...
          direccion:
            this.datos_de_contacto.controls.direccion.value! || undefined,
          telefono:
      ...
      this.router
        .navigateByUrl(`/admin/empleados`, { skipLocationChange: true })
        .then(() => {
